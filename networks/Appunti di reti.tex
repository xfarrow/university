...
\textbf{\textit{QUESTO CAPITOLO RICHIEDE IMPORTANTI RIVISITAZIONI E APPROFONDIMENTI}}\\\\\\
Il livello di trasporto fornisce la funzione di fornire servizi di comunicazione tra i processi applicativi in esecuzione su host differenti. Si tratta di estendere il servizio offerto dal livello di rete che invece fornisce servizi di comunicazione tra gli host.
...
Lato mittente, il livello di trasporto converte i messaggi che riceve da un processo applicativo in pacchetti a livello di trasporto detti segmenti, spezzando (se necessario) i messaggi in parti più piccole a aggiungendo informazioni header che a sua volta lo passa al livello di rete.
...
\begin{itemize}
	\item UDP (User Datagram Protocol): fornisce alle applicazioni non affidabile e non è orientato alla connessione. Fornisce un controllo degli errori molto semplice.
...
Il principale compito di TCP e UDP è l'estensione del servizio di consegna di IP tra sistemi periferici a quello di consegna tra processi in esecuzione su sistemi periferici. Questa funzionalità è detta multiplexing o demultiplexing a seconda della direzione.
...
Il compito di trasportare i dati dei segmenti a livello di trasporto verso la giusta socket viene detto demultiplexing. Viceversa, il compito di radunare dati da diverse socket e incapsulare ognuno con intestazioni a livello di trasporto per creare segmenti da passare al livello di rete viene detto multiplexing (CAPIRE O SCRIVERE MEGLIO).
...
Generalmente il lato client usa un numero casuale di porta mentre lato server si assegna un numero di porta specifico perché non vi è necessità di un numero di porta mittente specifico; infatti, il numero di porta di origine serve solo a specificare un indirizzo di ritorno. Si immagini che A voglia instaurare una connessione HTTP con B: allora A contatta B sulla porta 80 e mette nell'intestazione del segmento un numero di porta di origine casuale. Il server B, quando vuole rispondere ad A, farà assumere il valore del numero di porta di destinazione il numero di porta di origine che aveva ricevuto. 
...
Una differenza tra una socket UDP e una TCP risiede nel fatto che quest'ultima, a differenza della socket UDP, è identificata da quattro parametri: indirizzo IP di origine, porta di origine, indirizzo IP di destinazione, porta di destinazione. Eccezion fatta per i segmenti TCP che trasportano la richiesta per stabilire la connessione.
...
\subsubsection*{Web Server e TCP}
Non sempre esiste una corrispondenza uno a uno tra le socket e i processi. Infatti, si possono avere più socket collegate allo stesso processo.
...
Il checksum in UDP viene utilizzato per determinare se i bit del segmento UDP sono stati alterati durante il loro trasferimento.
...
\begin{itemize}
	...
	\item (B) Pacchetto duplicato (has\_seq sarebbe dovuto essere 0); invia ACK
	...
	\item (E) Pacchetto duplicato (has\_seq sarebbe dovuto essere 1); invia ACK
...
Notiamo che il mittente potrebbe ritrasmettere un pacchetto perché esso o l'ACK di ritorno ha sperimentato un ritardo notevolmente lungo ma non è andato perso. Ciò introduce la possibilità di pacchetti duplicati. Dunque il mittente non sa se un pacchetto  dati sia andato perduto, se sia stato smarrito un ACK o se uno dei due ha semplicemente subito un ritardo. In tutti questi casi l'azione intrapresa è sempre la stessa: ritrasmettere.
...
\begin{enumerate}
	\item Invocazione dall'alto: quando dall'alto si chiama \texttt{rdt\_send()} come prima cosa controlla se la finestra sia piena, ossia se vi siano N pacchetti in sospeso senza acknowledgment.
	\item Ricezione di un ACK: nel protocollo GBN l'ACK del pacchetto con numero di sequenza n verrà considerato come un acknlowledgment cumulativo che indica che tutti i pacchetti con numero di sequenza minore o uguale a n sono stati correttamente ricevuti
	\item Quando si verifica un timeout il mittente invia nuovamente tutti i pacchetti trasmessi e non riscontrati. Se si riceve un ACK ma ci sono ancora pacchetti in sospeso in attesa di ackwnowledgment il timer viene fatto ripartire.
\end{enumerate}
...
Il mittente ha un timer per ogni pacchetto che invia, ritrasmettendolo qualora non riceva un ACK nel tempo stabilito.

La finestra si muove in avanti verso il primo pacchetto non ancora ACK'd o non ancora usato quando riceve un ACK per \texttt{base}
...
\begin{itemize}
	\item Se B manda ad A 1000 byte, il prossimo segmento di A avrà un numero di acknowledgment di 1001
	\item Se B banda ad A 3 segmenti ma A riceve solo il primo e l'ultimo, allora A continuerà a mettere 1001 nel prossimo segmento che invierà a B.
\end{itemize}

Dato che TCP effettua l'acknowledgment dei byte fino all'ultimo byte che ha nel flusso, si dice che TCP offre acknowledgment cumulativi.

C'è una coppia (numero di sequenza, numero di acknowledgment) per il mittente e una coppia (numero di sequenza, numero di acknowledgment) per il destinatario, dato che la connessione è full-duplex.
...
\begin{enumerate}
	...
	\item Il destinatario risponde con la tripla \texttt{(Seq=79, Ack=43, data='C')}. Questo segmento ha un duplice obiettivo: confermare l'avvenuta ricezione del primo segmento (acknowledgment) e trasportare dati al client. Per questo motivo si dice che l'acknowledgment è "piggybacked"
...
TCP utilizza un meccanismo di timeut per recuperare i segmenti persi. Chiaramente, il timeout dovrebbe essere più grande del tempo di andata e ritorno sulla connessione (RTT). Ma di quanto deve essere maggiore? E come stimare RTT?
...
$$ \texttt{EstimatedRTT} = (1-\alpha) \times EstimatedRTT + \alpha \times SampleRTT $$ dove $\alpha$ è una costante fissa il cui valore raccomandato è 0,125.
...
che è simile alla formula precedente: con $\beta \times |\texttt{SampleRTT} - \texttt{EstimatedRTT}|$ vede quanto si discosta \texttt{SampleRTT} da \texttt{EstimatedRTT} e vi applica un fattore $\beta$ generalmente di 0,25.
...
Il valore iniziale di \texttt{TimeoutInterval} è pari a 1. \textbf{Dire che il valore viene raddoppiato quando si perde un pacchetto? Non ho capito}
...
\begin{enumerate}
	...
	\item Arrivo del segmento di acknowledgment: TCP confronta il valore contenuto nell'ACK con la propria variabile \texttt{Base} che rappresenta il numero di sequenza del più vecchio byte che non ha ancora ricevuto un acknowledgment. Conseguentemente \texttt{Base - 1} è il numero di sequenza dell'ultimo byte che si sa essere stato ricevuto correttamente. TCP usa acknowledgment cumulativi: Pertanto un ACK \texttt{y} conferma la ricezione di tutti i byte precedenti al byte numero \texttt{y}. A questo punto se ci sono altri segmenti in attesa di acknowledgment riavvia il timer altrimenti lo ferma.
\end{enumerate}
...
Quando il destinatario TCP rileva un buco nel flusso dei dati (ovvero un segmento mancante), il destinatario non può inviare un NAK esplicito per questi segmenti al mittente in quanto TCP non prevede ciò ma manda un ACK relativo all'ultimo byte che ha ricevuto correttamente e in ordine, duplicando così un ACK.

Nel caso in cui siano giunti tre ACK duplicati, il mittente effettua una cosiddetta ritramissione rapida, rispendendo il segmento mancante prima della scadenza del timer.

Queste sono le azioni intraprese dal server TCP:
\begin{enumerate}
...
\section*{3.6.1 Cause e costi della congestione}
...
Per fare questo, TCP fa tener traccia agli estremi della connessione una variabile aggiuntiva chiamata finestra di congestione, indicata con \texttt{cwnd}. Ricordandoci della variabile \texttt{rwnd} precedentemente menzionata, si avrà che TCP imporrà che la seguente diseguaglianza risulti sempre soddisfatta:
...
Quando si stabilisce una connessione TCP, il valore di \texttt{cwnd} viene impostato a 1 MSS e si incrementa di 1 MSS ogniqualvolta un segmento trasmesso riceve un ACK. Questo processo ha come effetto una velocità di trasmissione iniziale lenta per poi crescere esponenzialmente.
...
\begin{itemize}
	\item Se l'evento di perdita è stato causato da un timeout, si comporta allo stesso modo di Slow Start
	\item In caso di tre ACK duplicati, la rete continua a consegnare pacchetti, pertanto la risposta di TCP è meno drastica: imposta il valore \texttt{ssthresh} a  $\texttt{cwnd}/2$ e successivamente dimezza il valore di \texttt{cwnd} per poi entrare nello stato di fast recovery.
\end{itemize}

\subsubsection*{Fast recovery}
Durante la fase di fast recovery il valore di \texttt{cwnd} è incrementato di 1 MSS per ogni ACK duplicato ricevuto relativamente al segmento perso che ha causato l'entrata di TCP in modalità fast recovery.

\begin{itemize}
	\item Quando arriva un ACK per il segmento perso, TCP entra nello stato di congestion avoidance dopo aver ridotto il valore di \texttt{cwnd}
