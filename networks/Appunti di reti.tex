...
L'algoritmo di congestione di TCP presenta tre componenti: (1) slow start, (2) congestion avoidance e (3) fast recovery. Supponiamo che il mittente invii continuamente segmenti.
...
\begin{itemize}
	...
	\item In caso di tre ACK duplicati, la rete continua a consegnare pacchetti, pertanto la risposta di TCP è meno drastica: imposta il valore \texttt{ssthresh} a  $\texttt{cwnd}/2$ e successivamente dimezza il valore di \texttt{cwnd}, aggiungendo 3 (\texttt{cwnd} = $\frac{\texttt{cwnd}}{2}+3$), per poi entrare nello stato di fast recovery.
\end{itemize}
...
I router estraggono dai datagrammi dei campi di intestazione che usano come indice nella tabella di inoltro. Il valore trovato indica a quale interfaccia di uscita il segmento debba essere diretto.
\begin{table}[h!]
	\centering
	\begin{tabular}{|c|c|c|c|}
		\hline
...
Qui un controller remoto, fisicamente separato dagli altri router, calcola e distribuisce le tabelle di inoltro a tutti i router, facendo sì che la funzionalità di instradamento sia esterna al router a cui viene relagto solo il compito di inoltro.

Ciò è il cuore del cosiddetto "Software Defined Networking (SDN)" perché vi è un solo software che definisce la struttura logica della rete.
...
Sia $R_{switch}$ il tasso al quale i pacchetti vengono trasferiti dalla porta di input a quella di output e che i pacchetti arrivino alle porte di input a un tasso di $R_{line}$ e che ci siano $N$ porte. Se $R_{switch} \geq N \times R_{line}$ non ci sarà accodamento. Infatti anche supponendo il caso peggiore in cui arrivino $N$ pacchetti simultaneamente alle $N$ porte di input, avranno il tempo di essere smistati prima che ne arrivino altri. \textbf{e se destinati alla stessa porta?}

\subsubsection*{Accodamento in ingresso}
Che cosa avviene se la struttura di commutazione non è sufficientemente veloce?
In questo caso può verificarsi accodamento alle porte di ingresso. Sebbene sia possibile trasferire più pacchetti in parallelo, se due pacchetti sono destinati alla stessa porta di uscita uno dei due dovrà attendere. Se dietro al pacchetto che aspetta vi è un altro pacchetto destinato a una porta libera, esso dovrà comunque aspettare! Questo fenomeno è noto come "Blocco in testa alla coda", ovvero quando un pacchetto in ingresso deve comunque aspettare anche se la sua destinazione è libera.

\subsubsection*{Accodamento in uscita}
Supponiamo ancora che $R_{switch}$ sia $N \times R_{line}$ e che i pacchetti che giungono a ciascuna delle $N$ porte in ingresso sono destinati alla stessa porta di uscita. Dato che la porta di uscita può trasmettere solo un pacchetto in un intervallo stabilito, $N-1$ pacchetti dovranno mettersi in coda sul collegamento di uscita. Quindi è possibile che si formino code di pacchetti alle porte di uscita anche quando la struttura di commutazione è $N$ volte più veloce della velocità di linea delle porte. \textbf{CAPIRE QUESTO IN RELAZIONE A QUELLO CHE HA DETTO PRIMA}

In assenza di sufficiente memoria per inserire nel buffer il nuovo pacchetto occorrerà stabilire se scartare esso o uno dei pacchetti già in coda.
...
L'accodamento equo ponderato funziona in maniera simile ma a ciascuna classe $i$ è assegnata un peso $w_i$. La classe $i$ avrà la garanzia di ricevere una certa frazione di larghezza di banda \textbf{TODO: Capire!!!}
...
Ciò è dovuto al fatto che è l'interfaccia che fa parte di una rete, non l'host in sè: si può avere un host su più reti se ha più interfacce. Questo è il caso dei router che fanno da ponte tra una rete e l'altra.
...
Gli indirizzi non possono assulutamente essere scelti in modo arbitrario: una parte di esso è determinata dalla sottorete a cui è collegato.

La strategia di assegnazione degli indirizzi Internet è detta CIDR. Ogni indirizzo IP ha un ulteriore identificatore: la maschera di sottorete.
...
La dimensione della maschera di sottorete è scelta dall'amministratore del sistema, con limiti inferiori alla sua dimensione. Ad esempio se ad un amministratore viene assegnato di gestire lo spazio \texttt{192.168.1.0/24} potrà creare al suo interno sottoreti di dimensione da 25 a 31 bit.

Prima del CIDR gli indirizzi IP erano suddivisi in 3 classi: A,B e C con maschere di sottorete fisse rispettivamente di 8,16 e 24 bit. Ciò era poco flessibile.
