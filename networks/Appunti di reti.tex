...
Il sottoscritto consiglia di cercare in rete il funzionamento dell'algoritmo di Dijkstra: molto più semplice capirlo graficamente. Anche sul quaderno sono presenti degli esempi.
...
La formula sta a indicare che il costo minimo da $x$ a $y$ è $w(x,v) + \delta(v,y)$ calcolato su tutti i nodi $v$ adiacenti a $x$. Si noti come questa sia una formula ricorsiva in quando $\delta(v,y)$ sarà calcolata allo stesso modo.
\\
\\
...
Nel DV ciascun nodo dialoga solo con i vicini direttamente connessi, informandoli delle stime da sè stesso a tutti gli altri nodi della rete.

Nell'LS ciascun nodo dialoga con tutti gli altri nodi della rete, ma comunica loro solo i costi dei collegamenti ad esso direttamente connessi.

Vediamo un confronto tra i due:
\begin{itemize}
	\item Complessità dei messaggi: In LS ogni nodo deve comunicare il costo dei collegamenti ad esso connessi a tutti gli altri nodi della rete. Ciò implica l'invio di $|V| * |E|$ messaggi. In DV il numero di messaggi può variare
	\item LS è un algoritmo $O(|N|^2)$ che richiede lo scambio di $|V| * |E|$ messaggi. La velocità di convergenza di DV può variare e presenta problemi quali cicli di instradamento e conteggio all'infinito
	\item Robustezza: I calcol
\end{itemize}
