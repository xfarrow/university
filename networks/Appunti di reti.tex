...
Nonostante una struttura di commutazione molto veloce, è possibile accodamento in uscita. Perché? Dato che la porta di uscita può trasmettere solo un pacchetto alla volta, $N-1$ pacchetti dovranno mettersi in coda sul collegamento di uscita prima di essere trasmessi. Inoltre l'uscita può trasmettere solo a $R_{line}$ e quindi la coda tenderà a crescere indefinitamente. Ciò dimostra che è possibile che si formino code di pacchetti alle porte di uscita anche quando la struttura di commutazione è $N$ volte più veloce della velocità di linea delle porte.

\subsubsection*{Accodamento in ingresso}
Se la struttura di commutazione è abbastanza veloce e l'arrivo di pacchetti è adeguatamente cadenziato, non si formeranno code nella singola porta di input, tuttavia è possibile che un pacchetto debba mettersi in attesa lo stesso perché sta aspettando il trasferimento di un pacchetto da un'altra porta di input verso la propria porta di output.

Facciamo un esempio: due pacchetti su due porte di ingresso diverse devono uscire sulla stessa porta di uscita. La porta di uscita non può riceverle entrambe contemporaneamente quindi uno dei pacchetti deve rimanere in attesa.
